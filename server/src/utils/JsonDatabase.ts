...
export class JsonDatabase implements CrudInterface {
  ...
  private ensureArrayExists(table: Table): void {
    ...
    if (!this.internal.exists(path)) this.internal.push(path, [])
  }
  ...
  public async insert<T>(
    ...
    console.log("arg", arg)
    const existing: (T & { id: number })[] = this.internal.getData(
      JsonDatabase.makePath(table),
    )
  ...
  public async select<T>(
    ...
    try {
      ...
      if (typeof arg === "number") {
        ...
        return clone(
          this.internal.getData(JsonDatabase.makeArrayIndexPath(table, index)),
        )
      }

      const values =
        this.internal.filter<T>(
          JsonDatabase.makePath(table),
          this.makeFilter(arg),
        ) || null
  ...
  public async update<T>(
    ...
    const values = this.internal.filter<{ id: number }>(
      JsonDatabase.makePath(table),
      this.makeFilter(arg),
    )
    ...
    values.forEach((v) =>
      this.internal.push(
        JsonDatabase.makeArrayIndexPath(table, v.id),
        value,
        false,
      ),
    )
